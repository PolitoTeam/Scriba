...
Home::Home(QWidget *parent, Client *client)
    : QWidget(parent), ui(new Ui::Home), client(client) {
  ui->setupUi(this);
...
void Home::on_pushButtonNewFile_clicked() {
  bool ok;
  QString text = QInputDialog::getText(this, tr("New File"), tr("File name:"),
...
void Home::on_pushButtonSharedLink_clicked() {
  bool ok;
  QString text =
      QInputDialog::getText(this, tr("Shared Link"), tr("Paste link here:"),
                            QLineEdit::Normal, tr(""), &ok);
  if (ok && !text.isEmpty()) {
...
void Home::on_pushButtonOpenFile_clicked() { client->getFiles(false); }

void Home::on_pushButtonOpenShared_clicked() { client->getFiles(true); }
...
void Home::showActiveFiles(bool shared) {
  ...
  bool ok;
  QString item = QInputDialog::getItem(this, tr("Open File"), tr("Files:"),
                                       items, 0, false, &ok);
  if (ok && !item.isEmpty()) {
