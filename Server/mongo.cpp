...
void Mongo::connect() { db = conn["editor"]; }
...
void Mongo::upsertImage(QString email, const QByteArray &image) {
  ...
  collection.update_one(
      document << "email" << email.replace(".", "!").toStdString() << finalize,
      document << "$set" << open_document << "image" << bson_img
...
DatabaseError Mongo::signup(const QString &username, const QString &password) {
  ...
  try {
    ...
    auto cursor = collection.find_one_and_update(
        session, document << "username" << username.toStdString() << finalize,
        document << "$set" << open_document << "lock" << bsoncxx::oid()
                 << close_document << finalize);
    ...
    if (crypto_pwhash_str(hashed_password, password_char, strlen(password_char),
    ...
    bsoncxx::document::value doc = builder
                                   ...
                                   << "password" << hashed_password << finalize;
...
DatabaseError Mongo::login(const QString &username, const QString password,
  ...
  try {
    ...
    bsoncxx::builder::stream::document document{};
    auto cursor = collection.find_one(
        document << "username" << username.toStdString() << finalize);
  ...
  try {
    ...
    collection.update_one(
        ...
        document << "$set" << open_document << "nickname"
                 << nickname.toStdString() << close_document << finalize);
...
DatabaseError Mongo::updatePassword(const QString &username,
  ...
  try {
    ...
    collection.update_one(
        session, document << "username" << username.toStdString() << finalize,
        document << "$set" << open_document << "password" << hashed_newpassword
                 << close_document << finalize);
...
DatabaseError Mongo::getFiles(const QString &username,
  ...
  try {
    ...
    if (!shared) {
      ...
      auto cursor = collection.find_one(
          document << "username" << username.toStdString() << finalize, opts);
...
DatabaseError Mongo::newFile(const QString &username, const QString &filename,
  ...
  try {
    ...
    cursor = collection.find_one_and_update(
        session,
        document << "username" << username.toStdString() << "files"
                 << open_document << "$elemMatch" << open_document << "name"
                 << filename.toStdString() << close_document << close_document
                 << finalize,
        document << "$set" << open_document << "lock" << bsoncxx::oid()
                 << close_document << finalize);
    ...
    while (alreadyExisitingLink) {
      ...
      auto cursor = collection.find_one_and_update(
          session,
          document << "files" << open_document << "$elemMatch" << open_document
                   << "link" << sharedLink.toStdString() << close_document
                   << close_document << finalize,
          document << "$set" << open_document << "lock" << bsoncxx::oid()
                   << close_document << finalize);
    ...
    collection.update_one(
        session, document << "username" << username.toStdString() << finalize,
        document << "$push" << open_document << "files" << open_document
                 << "name" << filename.toStdString() << "link"
                 << sharedLink.toStdString() << close_document << close_document
                 << finalize);
...
DatabaseError Mongo::getSharedLink(const QString &author,
  ...
  try {
    ...
    mongocxx::options::find opts{};
    opts.projection(document << "files" << open_document << "$elemMatch"
                             << open_document << "name"
                             << filename.toStdString() << close_document
                             << close_document << finalize);

    auto cursor = collection.find_one(
        document << "username" << author.toStdString() << "files"
                 << open_document << "$elemMatch" << open_document << "name"
                 << filename.toStdString() << close_document << close_document
                 << finalize,
        opts);
...
DatabaseError Mongo::getFilenameFromSharedLink(const QString &sharedLink,
  ...
  try {
    ...
    opts.projection(document << "username" << 1 << "files" << open_document
                             << "$elemMatch" << open_document << "link"
                             << sharedLink.toStdString() << close_document
                             << close_document << finalize);
    ...
    filename =
        QString::fromStdString(name) + "," + QString::fromStdString(owner);

    collection.update_one(
        session, document << "username" << user.toStdString() << finalize,
        document << "$addToSet" << open_document << "shared_with_me"
                 << open_document << "filename" << name << "owner" << owner
...
bsoncxx::types::value Mongo::getObjectID(const QString &filename, bool &found) {
  ...
  auto cursor = collection.find_one(
      document << "filename" << filename.toStdString() << finalize, opts);
